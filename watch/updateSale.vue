	...
	<view class="sale-product-container">
		...
		<view class="sale-msg">
			...
			<uni-forms labelWidth="60" labelAlign="right" ref="saleWatchForm" :modelValue="saleWatchData"
				...
				<uni-forms-item label="顾客姓名" name="customer">
		...
		<view class="sale-buttton" v-if="role != 'channel'">
				...
				uni.showModal({
					...
					success: (ret) => {
						if (ret.confirm) {
							...
							uni.request({
								url: this.$baseUrl + "/newWatch/api/sellOrderDel?id=" + this
			...
			getSaleMsg() {
				uni.request({
					...
					complete: (res) => {
						...
						this.saleWatchData.channelId = msg.channelId;
						this.saleWatchData.customerId = msg.customerId;
				...
				uni.request({
					...
					complete: (ret) => {
						...
						if (ret.data.length > 0){
							this.channelList = ret.data;
							
							if(this.saleWatchData.channelId){
								for(let i = 0; i < this.channelList.length; ++i){
									if(this.saleWatchData.channelId == this.channelList[i].id){
										this.channelIdx = i;
										break;
									}
								}
				...
				uni.request({
					...
					complete: (ret) => {
						...
						if (ret.data.length > 0){
							...
							if(this.saleWatchData.customerId){
								for(let i = 0; i < this.customerList.length; ++i){
									if(this.saleWatchData.customerId == this.customerList[i].id){
										this.customerIdx = i;
										break;
									}
								}
							...
							console.log('this.saleWatchData.customerId', this.saleWatchData.customerId, this.customerIdx)
						}
