<template>
	<view>
		...
		<view v-if="role == 'admin' || role == 'seller'" class="action">
			...
			<navigator class="sale" :url="'../watch/addSale?code='+userNote">添加销售单</navigator>
		</view>
			...
			<view class="item" v-for="(msg, index) in msgList" :key="index" :id="'msg_' + index">
				<view style="margin-bottom: 20rpx; font-size: 28rpx">
					...
					<view v-if="isSelf(msg.sender)" style="display: flex">
						...
						<text @longpress="copyItem(msg.contentText)" v-if="msg.contentType == 0" class="msg" style="background-color: #1ECC99;">
