	...
	export default {
		...
		data() {
			return {
				...
				options: [ //左滑的操作列表
					{
						color: '', //字体颜色，非必须，默认 #ffffff
						bgColor: '#fff', //背景颜色，非必须，默认 #409EFF
						text: '', //展示文字
						width: 0, //按钮宽度 默认120rpx 适合两个字的宽度 这个宽度按需调整
					}
				],
				threshold: 50,
			};
		...
		onLoad(options) {
			this.userId = options.userId.replace('TopTime_', '');
			this.favorites();
		},
		onReady() {
		...
		methods: {
			...
			// 获取浏览记录列表 tempChat_CAZ3JDJDDf9E4L6cXc1AbbZXeLOBdOJK
			favorites() {
				uni.showLoading({
					title: "加载中...",
				});
				uni.request({
					url: this.$baseUrl + "/newWatch/api/browseRecords?userId=" + this.userId,
					header: {
