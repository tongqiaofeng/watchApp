<template>
	<view class="password-container-main">
		<view class="password-container">
			...
			<view class="login-main">
				<uni-forms ref="pswForm" :modelValue="pswData" :rules="pswRules">
					<uni-forms-item name="userName">
						<view>
							<text class="phone-num"> 用户名 </text>
							<view class="phone-main">
								<image class="phone-img" src="../static/imgs/mine/login/user.png" mode="aspectFill">
								</image>
								<view class="login-line"></view>
								<input type="text" class="phone-input" v-model="pswData.userName"
									placeholder="请输入用户名" />
							</view>
						</view>
					</uni-forms-item>
					<uni-forms-item name="mailbox">
						<view>
							<text class="phone-num"> 邮箱 </text>
							<view class="phone-main">
								<image class="phone-img" src="../static/imgs/mine/login/phone.png" mode="aspectFill">
								</image>
								<view class="login-line"></view>
								<input type="text" class="phone-input" v-model="pswData.mailbox" placeholder="请输入邮箱"
									@blur="emailBlur" />
							</view>
						</view>
					</uni-forms-item>
					<uni-forms-item name="code">
						<view>
							...
							<view class="phone-main code-main">
								...
								<view class="phone-main-left">
									<view class="login-line"></view>
									<button type="text" :plain="true" size="mini" style="
                      border: none;
                      padding: 0;
                      height: 17px;
                      line-height: 17px;
			...
			<view class="login-button" @click="updatePsw">
				确认修改
			</view>
			<navigator url="../common/login" hover-class="none" class="login-bottom">
		...
		methods: {
			...
			emailBlur() {
				if (this.pswData.mailbox != "")
					this.pswRules.code.rules[0].required = true;
				else
			...
			updatePsw() {
				this.$refs.pswForm
					...
					.then((res) => {
						...
						if (this.pswData.userName == "" && this.pswData.mailbox == "") {
							uni.showToast({
								icon: "none",
								title: "请输入用户名/邮箱"
							})
							...
							uni.request({
								method: "POST",
								url: this.$baseUrl + "/newWatch/api/findPsw",
								header: {
									"content-type": "application/json",
								},
								data: {
									username: this.pswData.userName,
									mailbox: this.pswData.mailbox,
									code: this.pswData.code,
									psw: this.pswData.newPsw,
								},
								complete: (res) => {
									uni.hideLoading();
									console.log("忘记密码");
