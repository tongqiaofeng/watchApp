		...
		<view v-if="detail.id">
			<view class="mainPrice">
				<view v-if="userType == 1 || userType == 0" class="priceItem">
					<view class="currency">{{detail.settleCurrency}}</view>
					<view class="price">{{getPrice(detail.settleMoney, '', '', 0)}}</view>
				</view>
			</view>
			...
			<view class="product-parameter">
				...
				<view v-if="detail.foreignMoney > 0 && userType == 1" class="parameter-every">
					<text>外币金额</text>
					<text>{{getShowValue(getPrice(detail.foreignMoney, detail.foreignCurrency, '', 0))}}</text>
				</view>
				
				<view v-if="detail.foreignToSettleRate" class="parameter-every">
					<text>汇率</text>
					<text>{{getShowValue(detail.foreignToSettleRate)}}</text>
				</view>
				...
				<view v-if="detail.watchList.length == 0 && detail.productDes != ''" class="parameter-every">
			...
			<view v-if="detail.watchList.length > 0" class="product-parameter" style="padding-bottom: 160rpx">
				...
				<view v-for="(bag, index) in detail.watchList" :key="index" class="parameter-every">
					<text style="flex: 1;">{{bag.stockNo}}</text>
					<text style="flex: 1; text-align: center;">{{getShowValue(getPrice(bag.foreignMoney, bag.foreignCurrency, '', 0))}}</text>
					<text style="flex: 1; text-align: right;">{{getShowValue(getPrice(bag.settleMoney, bag.settleCurrency, '', 0))}}</text>
				</view>
	...
	export default {
		...
		methods: {
			...
			getDetails(id) {
				...
				uni.request({
					url: this.$baseUrl + "/api/watch/stock/billInfo?id=" + id,
					header: {
