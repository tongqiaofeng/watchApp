...
function initData(vueOptions, context) {
  ...
  if (typeof data === 'function') {
    try {
      ...
      if (Object({"VUE_APP_DARK_MODE":"false","VUE_APP_NAME":"TopTime","VUE_APP_PLATFORM":"mp-weixin","NODE_ENV":"development","BASE_URL":"/"}).VUE_APP_DEBUG) {
...
/***/ 26:
/*!***********************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/pages.json ***!
  \***********************************************************/
/*! no static exports found */
/***/ (function(module, exports) {
...
/***/ 307:
/*!*********************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/qiun-data-charts/js_sdk/u-charts/u-charts.js ***!
  \*********************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
          ...
          if (eachSeries.style == 'curve') {
            for (var _j9 = 0; _j9 < points.length; _j9++) {
              ...
              if (_j9 > 0 && _item22.x > leftSpace && _item22.x < rightSpace) {
                ...
                context.bezierCurveTo(ctrlPoint.ctrA.x, ctrlPoint.ctrA.y, ctrlPoint.ctrB.x, ctrlPoint.ctrB.y, _item22.x, _item22.y);
              }
            }
          } else {
...
/***/ 33:
/*!******************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/js/chat.js ***!
  \******************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
var Chat = /*#__PURE__*/function () {
  function Chat(chatServerUrl, appID, userListRoute, userChatRoute, chatTabIdx) {
  ...
  (0, _createClass2.default)(Chat, [{
    key: "setUserInfo",
    value: function setUserInfo(userId, token, userName, headPic) {
      ...
      if (this.userId != userId || this.token != token) {
        ...
        if (userId.length > 0) this.connectServer();else this.closeConnect();
    ...
    key: "isSelf",
    value: function isSelf(userId) {
      //alert(this.userId + '_'+userId);
      if (this.userId == userId) return true;else return false;
    }
  }, {
    key: "setCurChatUserId",
    value: function setCurChatUserId(userId) {
    ...
    value: function connectServer() {
      ...
      if (this.g_pingpangTimes == null) {
        this.g_pingpangTimes = setInterval(function () {
          that.checkinServer();
          var curTime = new Date().getTime();
          if ((curTime - _this.g_socketLiveTime) / 1000 > 30) {
            //连续三次没收到心跳包，就重连
            that.connectServer();
          }
        }, 10000);
    ...
    key: "reConnectServer",
    value: function reConnectServer() {
      this.g_socketLiveTime = 0;
    }
    ...
    value: function updateUserInfo() {
      if (this.isUpdateUserInfo == false) {
        var note = uni.getStorageSync("superiorInviteCode");
        if (!note) note = '';
        var msg = {
          type: "updateUserInfo",
    ...
    key: "updateUserSetting",
    value: function updateUserSetting(offLineMsg) {
      var msg = {
    ...
    key: "recvFromServer",
    value: function recvFromServer() {
      var _this2 = this;
      uni.onSocketMessage(function (res) {
        _this2.g_socketLiveTime = new Date().getTime();
        ...
        if (data.type == 'checkin-success') {
          ...
          data.msgList = [];
          _this2.userList.push(data);
        } else if (data.type == 'historyAdmin') {
          _this2.adminList.push(data);
        } else if (data.type == 'history') {
          var userIdx = _this2.findUser(_this2.curChatUserId);
          if (userIdx > -1) {
            _this2.userList[userIdx].msgList.unshift(data);
          }
        } else if (data.type == 'message') {
          //更新用户列表与消息列表
          _this2.recvNewMsg(data);
          _this2.updatePage('chat');
          _this2.updatePage('list');
        } else if (data.type == 'messageAdmin') {
          _this2.adminList.push(data);
        } else if (data.type == 'user') {
          //更新用户头像
          _this2.updateUserPic(data);
        } else if (data.type == 'historyOverview-end') {
          _this2.updatePage('list');
          if (_this2.g_firstInit == true) {
            var page = _this2.getCurPage();
            if (page == 'chat') {
              _this2.getMsgHistoryCmd(_this2.curChatUserId, false);
            }
          }
          _this2.g_firstInit = false;
        } else if (data.type == 'history-end') {
          _this2.updatePage('chat');
          var _userIdx = _this2.findUser(_this2.curChatUserId);
          if (_userIdx > -1) {
            if (_this2.userList[_userIdx].num > 0) _this2.readMsgCmd(_this2.curChatUserId);
            _this2.userList[_userIdx].msgListSendMark = true;
          }
        } else if (data.type == 'read-success') {
          _this2.readMsgSuccess(data);
          _this2.updatePage('list');
        } else if (data.type == 'delete-success') {
          _this2.updateMsgState(data);
          _this2.updatePage('list');
          _this2.updatePage('chat');
          ...
          _this2.isUpdateUserInfo = true;
          _this2.UserChatSetting.offLineMsg = data.off_line_msg;
    ...
    key: "recvNewMsg",
    value: function recvNewMsg(data) {
      var page = this.getCurPage();
      if (data.sender == this.userId) {
        //自己发的消息

        //更新用户列表
        var userIdx = this.findUser(data.receiver);
        if (userIdx > -1) {
          //更新该用户
        ...
        //别人发给我的消息
        //更新用户列表
        var _userIdx2 = this.findUser(data.sender);
        var userName = data.sender;
        if (_userIdx2 > -1) {
          userName = this.userList[_userIdx2].username;
          //用户列表已有该用户，更新该用户
          this.userList[_userIdx2].contentType = data.contentType;
          this.userList[_userIdx2].contentText = data.contentText;
          this.userList[_userIdx2].time = data.time;
          this.userList[_userIdx2].id = data.id;
          //将消息添加到消息列表里
          data.type = 'history';
          data.readStatus = 0;
          this.userList[_userIdx2].msgList.push(data);
          if (page == 'chat' && data.sender == this.curChatUserId) {
            ...
            ++this.userList[_userIdx2].num;
          }

          //把元素移动到前面
          if (_userIdx2 > 0) {
            var _user = this.userList[_userIdx2];
            this.userList.splice(_userIdx2, 1);
            this.userList.unshift(_user);
          }
        } else {
    ...
    key: "findMsg",
    value: function findMsg(userIdx, msgInfo) {
      for (var i = this.userList[userIdx].msgList.length - 1, j = 0; i >= 0 && j < 20; --i, ++j) {
    ...
    value: function findUser(userId) {
      for (var i = 0; i < this.userList.length; ++i) {
        if (this.userList[i].userId == userId) return i;
      }
      return -1;
    }
    ...
    key: "getUserNote",
    value: function getUserNote(userId) {
      var note = '';
      var userIdx = this.findUser(userId);
      if (userIdx > -1) {
        note = this.userList[userIdx].note;
      }
      return note;
    }
  }, {
    key: "admin_findmsg",
    value: function admin_findmsg(id) {
      console.log("查找系统id", this.adminList);
      for (var i = 0; i < this.adminList.length; ++i) {
        if (this.adminList[i].id == id) return i;
      }
      return -1;
    }
    //发送消息
    ...
    key: "updateUserListCmd",
    value: function updateUserListCmd() {
      var msg = {
    ...
    key: "updateAdminListCmd",
    value: function updateAdminListCmd() {
      var msg = {
        type: "historyAdmin"
      };
      this.adminList = [];
    ...
    key: "deleteAdminList",
    value: function deleteAdminList(data) {
      var msg = {
        type: "deleteAdmin",
        id: data.id
      };
    ...
    key: "sendAdminList",
    value: function sendAdminList(type, content) {
      var msg = {
        type: "messageAdmin",
        receiver: userId,
        contentType: type,
    ...
    key: "updateUserMsg",
    value: function updateUserMsg(userId) {
      var userIdx = this.findUser(userId);
      if (userIdx > -1) {
        if (this.userList[userIdx].msgList.length == 0 || !this.userList[userIdx].msgListSendMark) this.getMsgHistoryCmd(userId, false);else {
          this.updatePage('chat');
        }
        if (this.userList[userIdx].num > 0) this.readMsgCmd(userId);
      }
    }

    //发送提取聊天记录命令:loadMore
  }, {
    key: "getMsgHistoryCmd",
    value: function getMsgHistoryCmd(userId) {
      var loadMore = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      var msgStartId = 0;
      var userIdx = this.findUser(userId);
      if (userIdx > -1) {
        if (loadMore) {
    ...
    key: "sendMsgToUserCmd",
    value: function sendMsgToUserCmd(userId, content, type) {
      var msg = {
        type: "message",
        receiver: userId,
        contentType: type,
    ...
    key: "updateMsgState",
    value: function updateMsgState(data) {
      if (data.type == 'message') {
        var userIdx = this.findUser(data.receiver);
        if (userIdx > -1) {
          for (var i = this.userList[userIdx].msgList.length - 1; i >= 0; --i) {
            if (this.userList[userIdx].msgList[i].readStatus == -1) {
        ...
        //删除消息
        var _userIdx3 = this.findUser(data.sender);
        if (_userIdx3 > -1) {
          //别人撤回
          for (var _i = this.userList[_userIdx3].msgList.length - 1; _i >= 0; --_i) {
            if (this.userList[_userIdx3].msgList[_i].id == data.id) {
              this.userList[_userIdx3].msgList[_i].readStatus = -2;
              if (_i == this.userList[_userIdx3].msgList.length - 1) {
                this.userList[_userIdx3].contentText = '[对方撤回了一条消息]';
              }
              break;
            }
          }
        }
        _userIdx3 = this.findUser(data.receiver);
        if (_userIdx3 > -1) {
          //自己撤回
          for (var _i2 = this.userList[_userIdx3].msgList.length - 1; _i2 >= 0; --_i2) {
            if (this.userList[_userIdx3].msgList[_i2].id == data.id) {
              this.userList[_userIdx3].msgList[_i2].readStatus = -2;
              if (_i2 == this.userList[_userIdx3].msgList.length - 1) {
                this.userList[_userIdx3].contentText = '[你撤回了一条消息]';
              }
              break;
            }
          }
        ...
        this.adminList.splice(_userIdx4, 1);
      }
    }
    ...
    key: "readMsgCmd",
    value: function readMsgCmd(userId) {
      var msg = {
        type: "read",
        userId: userId
      };
    ...
    key: "readMsgSuccess",
    value: function readMsgSuccess(data) {
      var userIdx = this.findUser(data.userId);
      if (userIdx > -1) {
    ...
    key: "deleteMsgCmd",
    value: function deleteMsgCmd(userId, msgId) {
      var msg = {
        type: "delete",
        receiver: userId,
        id: msgId
    ...
    key: "getUserInfoCmd",
    value: function getUserInfoCmd(userId) {
      var msg = {
        type: "user",
        userId: userId
      };
    ...
    key: "updateUserPic",
    value: function updateUserPic(data) {
      var userIdx = this.findUser(data.id);
      if (userIdx > -1) {
        //更新该用户
        this.userList[userIdx].username = data.username;
        this.userList[userIdx].head_pic = data.head_pic;
        this.userList[userIdx].note = data.note;
      }
    }
    ...
    value: function getCurPage() {
      var pages = getCurrentPages(); // 获取栈实例
      var currentRoute = pages[pages.length - 1].route;
      if (currentRoute == this.userListRoute) return 'list';else if (currentRoute == this.userChatRoute) return 'chat';else return '';
    }
...
/***/ 406:
/*!****************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-icons/components/uni-icons/icons.js ***!
  \****************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 414:
/*!*******************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-forms/components/uni-forms/validate.js ***!
  \*******************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
var _inherits2 = _interopRequireDefault(__webpack_require__(/*! @babel/runtime/helpers/inherits */ 415));
var _possibleConstructorReturn2 = _interopRequireDefault(__webpack_require__(/*! @babel/runtime/helpers/possibleConstructorReturn */ 416));
var _getPrototypeOf2 = _interopRequireDefault(__webpack_require__(/*! @babel/runtime/helpers/getPrototypeOf */ 418));
...
/***/ 489:
/*!*****************************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-datetime-picker/components/uni-datetime-picker/i18n/index.js ***!
  \*****************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 490:
/*!****************************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-datetime-picker/components/uni-datetime-picker/i18n/en.json ***!
  \****************************************************************************************************************************/
...
/***/ 491:
/*!*********************************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-datetime-picker/components/uni-datetime-picker/i18n/zh-Hans.json ***!
  \*********************************************************************************************************************************/
...
/***/ 492:
/*!*********************************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-datetime-picker/components/uni-datetime-picker/i18n/zh-Hant.json ***!
  \*********************************************************************************************************************************/
/*! exports provided: uni-datetime-picker.selectDate, uni-datetime-picker.selectTime, uni-datetime-picker.selectDateTime, uni-datetime-picker.startDate, uni-datetime-picker.endDate, uni-datetime-picker.startTime, uni-datetime-picker.endTime, uni-datetime-picker.ok, uni-datetime-picker.clear, uni-datetime-picker.cancel, uni-calender.SUN, uni-calender.MON, uni-calender.TUE, uni-calender.WED, uni-calender.THU, uni-calender.FRI, uni-calender.SAT, default */
/***/ (function(module) {
...
/***/ 514:
/*!*******************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-search-bar/components/uni-search-bar/i18n/index.js ***!
  \*******************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 515:
/*!******************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-search-bar/components/uni-search-bar/i18n/en.json ***!
  \******************************************************************************************************************/
...
/***/ 516:
/*!***********************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-search-bar/components/uni-search-bar/i18n/zh-Hans.json ***!
  \***********************************************************************************************************************/
/*! exports provided: uni-search-bar.cancel, uni-search-bar.placeholder, default */
/***/ (function(module) {
...
/***/ 517:
/*!***********************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-search-bar/components/uni-search-bar/i18n/zh-Hant.json ***!
  \***********************************************************************************************************************/
/*! exports provided: uni-search-bar.cancel, uni-search-bar.placeholder, default */
/***/ (function(module) {
...
var _possibleConstructorReturn2 = _interopRequireDefault(__webpack_require__(/*! @babel/runtime/helpers/possibleConstructorReturn */ 416));
var _getPrototypeOf2 = _interopRequireDefault(__webpack_require__(/*! @babel/runtime/helpers/getPrototypeOf */ 418));
var _wrapNativeSuper2 = _interopRequireDefault(__webpack_require__(/*! @babel/runtime/helpers/wrapNativeSuper */ 554));
var _uniI18n = __webpack_require__(/*! @dcloudio/uni-i18n */ 22);
var _pages = _interopRequireDefault(__webpack_require__(/*! @/pages.json */ 556));
...
try {
  {
    var _e2 = __webpack_require__(/*! uni-stat-config */ 557).default || __webpack_require__(/*! uni-stat-config */ 557);
    T = _e2.appid;
  }
} catch (e) {}
var A = {};
...
var getPrototypeOf = __webpack_require__(/*! ./getPrototypeOf.js */ 418);
var setPrototypeOf = __webpack_require__(/*! ./setPrototypeOf.js */ 16);
var isNativeFunction = __webpack_require__(/*! ./isNativeFunction.js */ 555);
...
/***/ 556:
/*!****************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/pages.json?{"type":"origin-pages-json"} ***!
  \****************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
var _default = {
  "pages": [{
    ...
    "style": {
      ...
      "navigationBarBackgroundColor": "#85dbd0",
  ...
  "subPackages": [{
    ...
    "pages": [{
      ...
      "path": "newsDetail",
      "style": {
        "navigationBarTitleText": "精品腕表，尽在TopTime",
        "enablePullDownRefresh": true,
        "app-plus": {}
      }
    }, {
    ...
    "pages": [{
      ...
      "path": "billDetail",
      "style": {
        "navigationBarTitleText": "帐单详情",
        "enablePullDownRefresh": true
      }
    }, {
      "path": "watchList",
      "style": {
        "navigationBarTitleText": "手表销售列表",
        "enablePullDownRefresh": true
      }
    }]
  }],
  ...
  "tabBar": {
    ...
    "list": [{
      ...
      "selectedIconPath": "static/imgs/tabBar/index01.png"
    }, {
      "pagePath": "pages/classify",
      "text": "分类",
      "iconPath": "static/imgs/tabBar/classify.png",
      "selectedIconPath": "static/imgs/tabBar/classify01.png"
    }, {
      "pagePath": "pages/chatList",
      "text": "消息",
      "iconPath": "static/imgs/tabBar/message.png",
      "selectedIconPath": "static/imgs/tabBar/message01.png"
    }, {
      "pagePath": "pages/recycle",
      "text": "回收",
      "iconPath": "static/imgs/tabBar/recycle.png",
      "selectedIconPath": "static/imgs/tabBar/recycle01.png"
    }, {
      "pagePath": "pages/mine",
      "text": "我的",
...
/***/ 557:
/*!***************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/pages.json?{"type":"stat"} ***!
  \***************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 565:
/*!************************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-transition/components/uni-transition/createAnimation.js ***!
  \************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 58:
/*!******************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/js/time.js ***!
  \******************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
var _default = {
  ...
  ISO8601DateStr2Date: function ISO8601DateStr2Date(string) {
...
/***/ 585:
/*!***********************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-datetime-picker/components/uni-datetime-picker/util.js ***!
  \***********************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 600:
/*!*****************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-load-more/components/uni-load-more/i18n/index.js ***!
  \*****************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/***/ 601:
/*!****************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-load-more/components/uni-load-more/i18n/en.json ***!
  \****************************************************************************************************************/
...
/***/ 602:
/*!*********************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-load-more/components/uni-load-more/i18n/zh-Hans.json ***!
  \*********************************************************************************************************************/
/*! exports provided: uni-load-more.contentdown, uni-load-more.contentrefresh, uni-load-more.contentnomore, default */
/***/ (function(module) {
...
/***/ 603:
/*!*********************************************************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/uni_modules/uni-load-more/components/uni-load-more/i18n/zh-Hant.json ***!
  \*********************************************************************************************************************/
/*! exports provided: uni-load-more.contentdown, uni-load-more.contentrefresh, uni-load-more.contentnomore, default */
/***/ (function(module) {
...
/***/ 83:
/*!**************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/imgs/search/up.png ***!
  \**************************************************************************/
/*! no static exports found */
/***/ (function(module, exports) {

module.exports = "data:image/png;base64,"

/***/ }),

/***/ 84:
/*!****************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/imgs/search/down.png ***!
  \****************************************************************************/
/*! no static exports found */
/***/ (function(module, exports) {

module.exports = "data:image/png;base64,"

/***/ }),

/***/ 85:
/*!******************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/imgs/search/normal.png ***!
  \******************************************************************************/
/*! no static exports found */
/***/ (function(module, exports) {

module.exports = "data:image/png;base64,"

/***/ }),

/***/ 86:
/*!***************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/imgs/search/yes.png ***!
  \***************************************************************************/
/*! no static exports found */
/***/ (function(module, exports) {

module.exports = "data:image/png;base64,"

/***/ }),

/***/ 87:
/*!**************************************************************************!*\
  !*** G:/aa-tong/my-project/watch/watchApp_Web/static/imgs/search/no.png ***!
  \**************************************************************************/
/*! no static exports found */
/***/ (function(module, exports) {

module.exports = "data:image/png;base64,"
